...
public enum EMySQLConnectionProperty implements IHasName
{
  ...
  parseInfoCacheFactory ("parseInfoCacheFactory",
                         ...
                         new Version (5, 1, 1)),
  /**
   * Name of a class implementing com.mysql.cj.api.CacheAdapterFactory<String,
   * Map<String, String>>, which will be used to create caches for MySQL server
   ...
   * "ON DUPLICATE KEY UPDATE" clause. As a side effect, obtaining the
   * statement's generated keys information will return a list where normally it
   * wouldn't. Also be aware that, in this case, the list of generated keys
   * returned may not be accurate. The effect of this property is canceled if
   * set simultaneously with 'rewriteBatchedStatements=true'.
   * 
   * @since MySQL 5.1.32
   */
   ...
   * getters that took a String property. JDBC-4.0 clarified "column name
   * " to mean the label, as given in an "AS" clause and returned by
