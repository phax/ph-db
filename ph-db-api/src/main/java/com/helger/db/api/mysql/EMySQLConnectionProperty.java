...
public enum EMySQLConnectionProperty implements IHasName
{
  ...
  /**
   * A comma-delimited list of user-defined key:value pairs (in addition to
   * standard MySQL-defined key:value pairs) to be passed to MySQL Server for
   * display as connection attributes in the
   ...
   * available for use with MySQL Server version 5.6 or later only. Earlier
   * versions of MySQL Server do not support connection attributes, causing this
   * configuration option to be ignored. Setting connectionAttributes=none will
   * cause connection attribute processing to be bypassed, for situations where
   * Connection creation/initialization speed is critical.
   *
   * @since MySQL 5.1.25
   */
   ...
   * "com.mysql.cj.api.jdbc.interceptors.ConnectionLifecycleInterceptor" that
   * should notified of connection lifecycle events (creation, destruction,
   * commit, rollback, setCatalog and setAutoCommit) and potentially alter the
   * execution of these commands. ConnectionLifecycleInterceptors are
   * "stackable", more than one interceptor may be specified via the
   * configuration property as a comma-delimited list, with the interceptors
   ...
   * com.mysql.cj.api.mysqla.authentication.AuthenticationPlugin and which will
   * be used for authentication unless disabled by
   ...
   * com.mysql.cj.api.mysqla.authentication.AuthenticationPlugin which will be
   * used as the default authentication plugin (see below). It is an error to
   * use a class which is not listed in "authenticationPlugins" nor it is one of
   * the built-in plugins. It is an error to set as default a plugin which was
   * disabled with "disabledAuthenticationPlugins" property. It is an error to
   * set this value to null or the empty string (i.e. there must be at least a
   * valid default authentication plugin specified for the connection, meeting
   ...
   * actual charsets/collations from server each time connection establishes.
   ...
   * com.mysql.cj.api.mysqla.authentication.AuthenticationPlugin or mechanisms,
   * i.e. "mysql_native_password". The authentication plugins or mechanisms
   * listed will not be used for authentication which will fail if it requires
   * one of them. It is an error to disable the default authentication plugin
   * (either the one named by "defaultAuthenticationPlugin" property or the
  ...
  /**
   * If "disconnectOnExpiredPasswords" is set to "false" and password is expired
   * then server enters "sandbox" mode and sends ERR(08001,
   * ER_MUST_CHANGE_PASSWORD) for all commands that are not needed to set a new
   ...
   * default value (null), uses the value set in "characterEncoding" if there is
   * one, otherwise uses UTF-8 as default encoding. If the password contains
   * non-ASCII characters, the password encoding must match what server encoding
   * was set to when the password was created. For passwords in other character
   * encodings, the encoding will have to be specified with this property (or
   * with "characterEncoding"), as it's not possible for the driver to
   * auto-detect this.
   *
   * @since MySQL 5.1.7
   */
  ...
  /**
   * An implementation of com.mysql.cj.api.conf.ConnectionPropertiesTransform
   * that the driver will use to modify URL properties passed to the driver
  ...
  /**
   * What character encoding should the driver use when dealing with strings?
   ...
   * 'com.mysql.cj.api.io.SocketFactory' and have public no-args constructor.
   *
   * @since MySQL 3.0.3
   */
   ...
   * 'max_allowed_packet'. Also, due to an internal dependency with the property
  ...
  /**
   * Use SSL when communicating with the server (true/false), default is 'true'
   * when connecting to MySQL 5.5.45+, 5.6.26+ or 5.7.6+, otherwise default is
   * 'false'
   *
   * @since MySQL 3.0.2
   */
  ...
  /**
   * Require server support of SSL connection if useSSL=true? (defaults to
   * 'false').
   *
   * @since MySQL 3.1.0
   */
   ...
   * specified by the "clientCertificateKeyStore*" properties, rather than
   * system properties. Default is 'false' when connecting to MySQL 5.5.45+,
   * 5.6.26+ or 5.7.6+ and "useSSL" was not explicitly set to "true". Otherwise
   * default is 'true'
   *
   * @since MySQL 5.1.6
   */
  ...
  /**
   * If "useSSL" is set to "true", overrides the cipher suites enabled for use
   * on the underlying SSL sockets. This may be required when using external
   * JSSE providers or to specify cipher suites compatible with both MySQL
   * server and used JVM.
   *
   * @since MySQL 5.1.35
   */
  ...
  /**
   * Allows special handshake roundtrip to get server RSA public key directly
   * from server.
   *
   * @since MySQL 5.1.31
   */
   ...
   * authentication. If not specified, the public key will be retrieved from the
   * server.
   *
   * @since MySQL 5.1.31
   */
   ...
   * more memory efficient for some applications. Also the automatic closing of
   * the Statement and current ResultSet in Statement.closeOnCompletion() and
   * Statement.getMoreResults ([Statement.CLOSE_CURRENT_RESULT |
   * Statement.CLOSE_ALL_RESULTS]), respectively, ceases to happen. This
   * property automatically sets holdResultsOpenOverStatementClose=true.
   *
   * @since MySQL 3.1.7
   */
  ...
  /**
   * A comma-delimited list of classes that implement
   * "com.mysql.cj.api.jdbc.interceptors.StatementInterceptor" that should be
   ...
   * "current" interceptor will be passed on to the next in in the chain, from
   ...
   * Default escape processing behavior in non-prepared statements must be
   * defined with the property 'enableEscapeProcessing'.
   *
   * @since MySQL 3.1.12
   */
   ...
   * getProcedureColumns() methods, so they return metadata info for both stored
   * procedures and functions. JDBC4 was extended with the getFunctions() and
   * getFunctionColumns() methods and the expected behaviours of previous
   * methods are not well defined. For JDBC4 and higher, default 'true' value of
   * the option means that calls of DatabaseMetaData.getProcedures() and
   * DatabaseMetaData.getProcedureColumns() return metadata for both procedures
   * and functions as before, keeping backward compatibility. Setting this
   * property to 'false' decouples Connector/J from its pre-JDBC4 behaviours for
   * DatabaseMetaData.getProcedures() and
   * DatabaseMetaData.getProcedureColumns(), forcing them to return metadata for
   * procedures only.
   *
   * @since MySQL 5.1.26
   */
   ...
   * connected user can't access procedure bodies through "SHOW CREATE
   * PROCEDURE" or select on mysql.proc should the driver instead create basic
   * metadata (all parameters reported as INOUT VARCHARs) instead of throwing an
   * exception?
   *
   * @since MySQL 5.0.3
   */
  ...
  /**
   * Chunk size to use when sending BLOB/CLOBs via ServerPreparedStatements.
   * Note that this value cannot exceed the value of "maxAllowedPacket" and, if
   * that is the case, then this value will be corrected automatically.
   *
   * @since MySQL 3.1.9
   */
  ...
  /**
   * Send fractional part from TIMESTAMP seconds. If set to false, the
   * nanoseconds value of TIMESTAMP values will be truncated before sending any
   * data to the server. This option applies only to prepared statements,
  ...
  /**
   * Override detection/mapping of time zone. Used when time zone from server
   * doesn't map to Java time zone
   *
   * @since MySQL 3.0.2
   */
  ...
  /**
   * When using loadbalancing or failover, the number of times the driver should
   * cycle through available hosts, attempting to connect. Between cycles, the
   * driver will pause for 250ms if no servers are available.
   *
   * @since MySQL 5.1.6
   */
  ...
  /**
   * Number of queries to issue before falling back to the primary host when
   * failed over (when using multi-host failover). Whichever condition is met
   * first, 'queriesBeforeRetryMaster' or 'secondsBeforeRetryMaster' will cause
   * an attempt to be made to reconnect to the primary host. Setting both
   * properties to 0 disables the automatic fall back to the primary host at
   ...
   * reconnect to the primary host? Whichever condition is met first,
   * 'queriesBeforeRetryMaster' or 'secondsBeforeRetryMaster' will cause an
   * attempt to be made to reconnect to the master. Setting both properties to 0
   * disables the automatic fall back to the primary host at transaction
  ...
  /**
   * By default, a replication-aware connection will fail to connect when
   * configured master hosts are all unavailable at initial connection. Setting
   * this property to 'true' allows to establish the initial connection, by
   * failing over to the slave servers, in read-only state. It won't prevent
   * subsequent failures when switching back to the master hosts i.e. by setting
   * the replication connection to read/write state.
   *
   * @since MySQL 5.1.27
   */
  ...
  /**
   * By default, a replication-aware connection will fail to connect when
   * configured slave hosts are all unavailable at initial connection. Setting
   * this property to 'true' allows to establish the initial connection. It
   * won't prevent failures when switching to slaves i.e. by setting the
   * replication connection to read-only state. The property
   ...
   * live addition/removal of hosts from load-balancing pool. Enables JMX-based
   * management of replication connection groups, including live slave
   * promotion, addition of new slaves and removal of master or slave hosts from
  ...
  /**
   * Sets the grace period to wait for a host being removed from a load-balanced
   * connection, to be released when it is currently the active host.
   *
   * @since MySQL 5.1.39
   */
  loadBalanceHostRemovalGracePeriod ("loadBalanceHostRemovalGracePeriod", "15000", new Version (5, 1, 39)),
  /**
   * Replication-aware connections distribute load by using the master hosts
   * when in read/write state and by using the slave hosts when in read-only
   * state. If, when setting the connection to read-only state, none of the
   * slave hosts are available, an SQLExeception is thrown back. Setting this
   * property to 'true' allows to fail over to the master hosts, while setting
   * the connection state to read-only, when no slave hosts are available at
   * switch instant.
   *
   * @since MySQL 5.1.38
   */
   ...
   * called if the connection's count of commands sent to the server exceeds
   * this value.
   *
   * @since MySQL 5.1.6
   */
   ...
   * implement com.mysql.cj.api.jdbc.ha.LoadBalanceExceptionChecker interface,
   * and is used to inspect SQLExceptions and determine whether they should
   * trigger fail-over to another host in a load-balanced deployment.
   *
   * @since MySQL 5.1.13
   */
  ...
  /**
   * Name of a class implementing com.mysql.cj.api.CacheAdapterFactory, which
   * will be used to create caches for the parsed representation of client-side
   * prepared statements.
   *
   * @since MySQL 5.1.1
   */
   ...
   * String&gt;&gt;, which will be used to create caches for MySQL server
  ...
  /**
   * Stops checking if every INSERT statement contains the "ON DUPLICATE KEY
   * UPDATE" clause. As a side effect, obtaining the statement's generated keys
   * information will return a list where normally it wouldn't. Also be aware
   * that, in this case, the list of generated keys returned may not be
   * accurate. The effect of this property is canceled if set simultaneously
  ...
  /**
   * Sets the default escape processing behavior for Statement objects. The
   * method Statement.setEscapeProcessing() can be used to specify the escape
   * processing behavior for an individual Statement object. Default escape
   * processing behavior in prepared statements must be defined with the
  ...
  /**
   * Should the driver issue appropriate statements to implicitly set the
   * transaction access mode on server side when Connection.setReadOnly() is
   * called? Setting this property to 'true' enables InnoDB read-only potential
   * optimizations but also requires an extra roundtrip to set the right
   * transaction state. Even if this property is set to 'false', the driver will
   * do its best effort to prevent the execution of database-state-changing
   * queries. Requires minimum of MySQL 5.6.
   *
   * @since MySQL 5.1.35
   */
   ...
   * the entire batch includes INSERT statements. Please be aware using
   * rewriteBatchedStatements=true with INSERT .. ON DUPLICATE KEY UPDATE that
   * for rewritten statement server returns only one value as sum of all
   * affected (or found) rows in batch and it isn't possible to map it correctly
   * to initial statements; in this case driver returns 0 as a result of each
   * batch statement if total count was 0, and the Statement.SUCCESS_NO_INFO as
   * a result of each batch statement if total count was &gt; 0.
   *
   * @since MySQL 3.1.13
   */
  ...
  /**
   * The name of a class that implements "com.mysql.cj.api.log.Log" that will be
   * used to log messages to. (default is
   * "com.mysql.cj.core.log.StandardLogger", which logs to STDERR)
   *
   * @since MySQL 3.1.1
   */
   ...
   * com.mysql.cj.api.jdbc.ClientInfoProvider interface in order to support
   ...
   * com.mysql.cj.api.ProfilerEventHandler that will be used to handle
   ...
   * the driver try to use nanoseconds resolution if available (JDK &ge; 1.5)?
   *
   * @since MySQL 5.0.7
   */
  ...
  /**
   * Comma-delimited list of classes that implement
   * com.mysql.cj.api.exceptions.ExceptionInterceptor. These classes will be
   * instantiated one per Connection instance, and all SQLExceptions thrown by
   * the driver will be allowed to be intercepted by these interceptors, in a
   * chained fashion, with the first class listed as the head of the chain.
   *
   * @since MySQL 5.1.8
   */
  ...
  /**
   * Include the name of the current thread as a comment visible in "SHOW
   * PROCESSLIST", or in Innodb deadlock dumps, useful in correlation with
   ...
   * getters that took a String property. JDBC-4.0 clarified "column name " to
   * mean the label, as given in an "AS" clause and returned by
   * ResultSetMetaData.getColumnLabel(), and if no AS clause, the column name.
   * Setting this property to "true" will give behavior that is congruent to
   * JDBC-3.0 and earlier versions of the JDBC specification, but which because
   * of the specification bug could give unexpected results. This property is
   * preferred over "useOldAliasMetadataBehavior" unless you need the specific
